...
\begin{frame}
    \frametitle{与BA模型的比较}
    \begin{itemize}
        \item \textbf{目的:} 检验简单的生成模型是否能复现真实网络的特性。
        \item \textbf{模型:} Barabási-Albert (BA) 模型，采用“优先连接”机制。
        \item \textbf{参数:}
        \begin{itemize}
            \item $N = 116,408$ (与真实网络相同)
            \item $m=2$ (新节点连接到2个现有节点)
        \end{itemize}
    \end{itemize}
    \pause
    ...
    \begin{tabular}{l c c}
        \toprule
        \textbf{指标} & \textbf{真实网络} & \textbf{BA 模型} \\
        \midrule
        平均度 & 2.59 & 4.00 \\
        最大连通分量大小 & 1801 & 1 \\
        模块度 & 0.8525 & 0.5427 \\
...
\begin{frame}
    \frametitle{比较结论}
    \begin{itemize}
        \item \textbf{BA模型的成功之处:}
        \begin{itemize}
            \item 成功复现了真实网络的\textbf{无标度}特性，两者都呈现幂律度分布。
            \item 证明\textbf{优先连接}是此类社交网络演化的基本机制。
        \end{itemize}
        \pause
        \item \textbf{BA模型的局限性:}
        \begin{itemize}
            \item \textbf{未能形成核心社群:} 真实网络有1801个节点的核心连通分量，而BA模型是碎片化的。
            \item \textbf{未能产生强社区结构:} 真实网络的模块度极高 (0.85)，而BA模型则低得多 (0.54)。
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item \textbf{网络类型:}
        典型的\textbf{无标度}社交网络，规模庞大但结构松散。
        \pause
        \item \textbf{网络结构:}
        \begin{itemize}
            \item 存在核心“巨型连通分量”和大量外围小社群。
            \item 具有高度模块化的社区结构 (模块度 > 0.85)。
        \end{itemize}
        \pause
        \item \textbf{关键节点:}
        中心性分析识别出网络中的核心节点 (如\textbf{节点 88})。
        \pause
        \item \textbf{模型比较:}
        BA模型验证了\textbf{优先连接}机制，但过于简单，无法捕捉真实网络丰富的社区结构。
    \end{itemize}
\end{frame}
