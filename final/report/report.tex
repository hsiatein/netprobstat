...
本项目的分析主要包括网络的基本统计特性、关键节点的识别、社区结构的发现、网络整体的拓扑性质以及随机模型的模拟。

\section{网络分析结果}

\subsection{图的基本统计信息}
为了理解网络的基本性质，本项目首先计算了图的一系列基本的统计量。

\begin{itemize}
    \item \textbf{节点数量: 116408}: 网络中包含了超过11万个用户（节点）。
    \item \textbf{边数量: 150818}: 用户之间有15万次提及，构成了网络的所有边。
    \item \textbf{平均度: 2.59}: 每个用户平均与其他约2.6个用户直接相连。说明网络较为稀疏。
    \item \textbf{图不连通}: 整个网络并非一个单一的连通图，而是由多个独立的子图构成，并非所有用户都能通过网络路径相互到达。
    \item \textbf{连通分量: 110704}: 网络中存在大量的独立子图，绝大多数连通分量都非常小。
    \item \textbf{最大连通分量: 1801 节点}: 网络中最大的一个连通子图包含了1801个用户，代表网络中最核心的交流群体。
\end{itemize}

网络特征：规模庞大但结构松散，核心讨论由一个相对较小的最大连通分量主导，而外围则散布着大量孤立的讨论组(连通分量)。

\subsection{中心性分析}
本项目从四个不同的维度进行此网络的中心性分析：度中心性、介数中心性、紧密中心性和特征向量中心性。
...
节点88的度中心性远超其他节点，表明它可能是该话题的核心信息源，例如权威新闻机构。其他高中心性节点也是类似的关键角色，但影响力相对较小。
...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}
介数中心性高的节点是信息在不同社群之间传播的关键枢纽，高介数中心性的节点在连接不同用户群体方面起重要作用。节点88同样具有最高的介数中心性。
...
\begin{itemize}
    \item \textbf{社区数量: 10681}: 网络中存在大量的小社群，这与连通分量数量相符。
    \item \textbf{模块度: 0.8525}: 模块度值非常高，说明该网络具有明显的社区结构。
\end{itemize}
以下是网络中最大的5个社区及其规模:
\begin{table}[H]
    \centering
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{未能产生紧密的社区结构：} 真实网络的模块度高达0.85，说明其具有非常清晰的社区结构。相比之下，BA模型的模块度仅为0.54。
    \end{itemize}
\end{enumerate}
